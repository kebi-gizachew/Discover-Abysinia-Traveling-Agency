...
export const ContactForm = async (req, res)=>{
  try {
    const {name,email,phone, country, travelDate,destination, message}=await parseRequestBody(req)
    const newContactMessage = await Contact.create({name,email,phone,country,
    travelDate,
      destination,
      message,
    })
    sendJSON(res,201,{message: "Contact message submitted successfully",
